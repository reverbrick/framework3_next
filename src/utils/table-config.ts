...
// Function to generate TanStack Table columns from our table config
export function generateColumns<TData>(config: TableConfig): ColumnDef<TData>[] {
  return config.columns.map((column) => {
    ...
    if (column.type !== 'computed' && column.type !== 'checkbox' && column.type !== 'actions') {
      ;(col as any).accessorKey = column.id
    }

    // Add meta for sticky columns
    if (column.sticky) {
      col.meta = {
        className: 'sticky left-0 md:table-cell',
      }
    }
